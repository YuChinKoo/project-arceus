...
export default function RequestedTaskBoardThumbnail(props) {
    ...
    const [acceptRequest, {acceptLoading, acceptError}] = useMutation(RESPONSE_TASKBOARD_HELPER_REQUEST, {
        onError: (err) => {
            setQLoading(false);
            setErrorMessage(`${err}`);
    ...
    const [denyRequest, {denyLoading, denyError}] = useMutation(RESPONSE_TASKBOARD_HELPER_REQUEST, {
        onError: (err) => {
            setQLoading(false);
            setErrorMessage(`${err}`);
            console.log(`Error! ${err}`);
